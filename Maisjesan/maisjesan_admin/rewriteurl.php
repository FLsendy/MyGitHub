...
RewriteCond %{QUERY_STRING} ^(.*)$
RewriteRule ^(.*)/(\w+)-([0-9]+)\.html$ $1/signl=$2&page=$3
RewriteCond %{QUERY_STRING} ^(.*)$
...
RewriteCond %{QUERY_STRING} ^(.*)$
RewriteRule ^(\w+)-([0-9]+)\.html$ signl=$1&page=$2
RewriteCond %{QUERY_STRING} ^(.*)$
...
&lt;rule name="join"&gt;
	...
	&lt;action type="Rewrite" url="{R:1}/signl={R:2}&amp;amp;page={R:3}" /&gt;
&lt;/rule&gt;
&lt;rule name="joinshow"&gt;
...
rewrite ^([^\.]*)/(\w+)-([0-9]+)\.html$ $1/signl=$2&page=$3 last;
