...
function GetMgrAjaxType($tbname='', $type='', $id=0, $i=0)
{
	...
	while($row = $dosql->GetArray($id))
	{
		...
		{
			echo '<a href="javascript:;" onclick="GetType2(\''.$row['id'].'\',\''.$row["classname"].'\')">';
			for($p=1; $p<$i; $p++)
			{
				echo '　';
			}
...
function GetDiyField($type='',$id=0,$row='')
{
	...
	while($r = $dosql->GetArray())
	{
		...
		if(in_array($id,$catepriv))
		{
			...
			$reStr .= '><td height="35" align="right" width="25%">'.$r['fieldtitle'].'：</td><td width="75%">';
	

			//文本框
			if($r['fieldtype']=='varchar' or $r['fieldtype']=='int' or $r['fieldtype']=='decimal')
			{
				$reStr .= '<input type="text" name="'.$r['fieldname'].'" id="'.$r['fieldname'].'" class="class_input" value="'.$fieldvalue.'" />';
			...
			{
				$reStr .= '<textarea name="'.$r['fieldname'].'" id="'.$r['fieldname'].'" class="class_areatext" style="margin:7px 0;">'.$fieldvalue.'</textarea>';
				if(!empty($r['fieldcheck']))
				{
					$reStr .= '&nbsp;<span class="maroon">*</span>';
				}
			...
			{
				$reStr .= '<input type="text" name="'.$r['fieldname'].'" id="'.$r['fieldname'].'" class="class_input" value="'.$fieldvalue.'" />';
				$reStr .= '<span class="cnote"><span class="gray_btn" onclick="GetUploadify(\'uploadify\',\''.$r['fieldtitle'].'\',\'all\',\'all\',1,'.$cfg_max_file_size.',\''.$r['fieldname'].'\')">上 传</span></span>';
				if(!empty($r['fieldcheck']))
				{
					$reStr .= '&nbsp;<span class="maroon">*</span>';
				}
			...
			else if($r['fieldtype'] == 'datetime')
			{
				...
				$reStr .= '<style>#'.$r['fieldname'].'{background:url(templates/images/calendar.gif) 127px no-repeat;cursor:pointer;}</style>';
				$reStr .= '<input type="text" name="'.$r['fieldname'].'" id="'.$r['fieldname'].'" class="input_short" value="'.$dtime .'" readonly="readonly" />';
			...
			else if($r['fieldtype'] == 'mediumtext')
			{
				...
				$reStr .= '<script type="text/javascript">var editor_'.$r['fieldname'].';KindEditor.ready(function(K) {editor_'.$r['fieldname'].' = K.create(\'textarea[name="'.$r['fieldname'].'"]\', {allowFileManager : true,width:\'667px\',height:\'280px\'});});</script>';
			}
...
//验证栏目权限
function IsCategoryPriv($cid=0,$act='',$return='')
{
	global $dosql, $cfg_adminlevel;
